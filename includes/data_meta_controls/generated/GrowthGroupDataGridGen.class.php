<?php
	 ...
	 * listing a collection of GrowthGroup objects.  It includes
	 ...
	 * 
	 */
	class GrowthGroupDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this GrowthGroup datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the GrowthGroup object, or it can be any QQNode extending from QQN::GrowthGroup().
		 * 
		 ...
		 * sorting using two fields from the GrowthGroup object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from GrowthGroup
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intGroupId=<?=urlencode($_ITEM->GroupId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->GroupId))?>';
					break;
				default:
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = GrowthGroup::CountAll();
			}
			...
			$this->DataSource = GrowthGroup::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a GrowthGroup-based QQNode.
		 * It will also verify that it is a proper GrowthGroup-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'growth_group') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "growth_group".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'GroupId': return QQN::GrowthGroup()->GroupId;
				case 'Group': return QQN::GrowthGroup()->Group;
				case 'GrowthGroupLocationId': return QQN::GrowthGroup()->GrowthGroupLocationId;
				case 'GrowthGroupLocation': return QQN::GrowthGroup()->GrowthGroupLocation;
				case 'GrowthGroupDayTypeId': return QQN::GrowthGroup()->GrowthGroupDayTypeId;
				case 'MeetingBitmap': return QQN::GrowthGroup()->MeetingBitmap;
				case 'StartTime': return QQN::GrowthGroup()->StartTime;
				case 'EndTime': return QQN::GrowthGroup()->EndTime;
				case 'Address1': return QQN::GrowthGroup()->Address1;
				case 'Address2': return QQN::GrowthGroup()->Address2;
				case 'CrossStreet1': return QQN::GrowthGroup()->CrossStreet1;
				case 'CrossStreet2': return QQN::GrowthGroup()->CrossStreet2;
				case 'ZipCode': return QQN::GrowthGroup()->ZipCode;
