<?php
	 ...
	 * listing a collection of SearchQuery objects.  It includes
	 ...
	 * 
	 */
	class SearchQueryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SearchQuery datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SearchQuery object, or it can be any QQNode extending from QQN::SearchQuery().
		 * 
		 ...
		 * sorting using two fields from the SearchQuery object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SearchQuery
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SearchQuery::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from SearchQuery, given the clauses above
			$this->DataSource = SearchQuery::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SearchQuery-based QQNode.
		 * It will also verify that it is a proper SearchQuery-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'search_query') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "search_query".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SearchQuery()->Id;
				case 'Description': return QQN::SearchQuery()->Description;
				case 'SmartGroupId': return QQN::SearchQuery()->SmartGroupId;
				case 'SmartGroup': return QQN::SearchQuery()->SmartGroup;
				case 'PersonId': return QQN::SearchQuery()->PersonId;
				case 'Person': return QQN::SearchQuery()->Person;
				default: throw new QCallerException('Simple Property not found in SearchQueryDataGrid content: ' . $mixContent);
