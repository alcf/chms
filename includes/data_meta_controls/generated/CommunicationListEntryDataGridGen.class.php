<?php
	 ...
	 * listing a collection of CommunicationListEntry objects.  It includes
	 ...
	 * 
	 */
	class CommunicationListEntryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CommunicationListEntry datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CommunicationListEntry object, or it can be any QQNode extending from QQN::CommunicationListEntry().
		 * 
		 ...
		 * sorting using two fields from the CommunicationListEntry object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CommunicationListEntry
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = CommunicationListEntry::CountAll();
			}
			...
			$this->DataSource = CommunicationListEntry::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CommunicationListEntry-based QQNode.
		 * It will also verify that it is a proper CommunicationListEntry-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'communication_list_entry') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "communication_list_entry".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CommunicationListEntry()->Id;
				case 'FirstName': return QQN::CommunicationListEntry()->FirstName;
				case 'MiddleName': return QQN::CommunicationListEntry()->MiddleName;
				case 'LastName': return QQN::CommunicationListEntry()->LastName;
				case 'Email': return QQN::CommunicationListEntry()->Email;
				default: throw new QCallerException('Simple Property not found in CommunicationListEntryDataGrid content: ' . $mixContent);
