<?php
	 ...
	 * listing a collection of Phone objects.  It includes
	 ...
	 * 
	 */
	class PhoneDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Phone datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Phone object, or it can be any QQNode extending from QQN::Phone().
		 * 
		 ...
		 * sorting using two fields from the Phone object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Phone
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Phone::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Phone, given the clauses above
			$this->DataSource = Phone::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Phone-based QQNode.
		 * It will also verify that it is a proper Phone-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'phone') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "phone".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Phone()->Id;
				case 'PhoneTypeId': return QQN::Phone()->PhoneTypeId;
				case 'AddressId': return QQN::Phone()->AddressId;
				case 'Address': return QQN::Phone()->Address;
