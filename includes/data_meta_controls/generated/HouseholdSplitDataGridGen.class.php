<?php
	 ...
	 * listing a collection of HouseholdSplit objects.  It includes
	 ...
	 * 
	 */
	class HouseholdSplitDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this HouseholdSplit datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the HouseholdSplit object, or it can be any QQNode extending from QQN::HouseholdSplit().
		 * 
		 ...
		 * sorting using two fields from the HouseholdSplit object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from HouseholdSplit
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = HouseholdSplit::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from HouseholdSplit, given the clauses above
			$this->DataSource = HouseholdSplit::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a HouseholdSplit-based QQNode.
		 * It will also verify that it is a proper HouseholdSplit-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'household_split') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "household_split".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::HouseholdSplit()->Id;
				case 'HouseholdId': return QQN::HouseholdSplit()->HouseholdId;
				case 'Household': return QQN::HouseholdSplit()->Household;
				case 'SplitHouseholdId': return QQN::HouseholdSplit()->SplitHouseholdId;
				case 'SplitHousehold': return QQN::HouseholdSplit()->SplitHousehold;
				case 'DateSplit': return QQN::HouseholdSplit()->DateSplit;
				default: throw new QCallerException('Simple Property not found in HouseholdSplitDataGrid content: ' . $mixContent);
