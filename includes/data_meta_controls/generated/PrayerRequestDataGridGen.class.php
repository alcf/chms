<?php
	 ...
	 * listing a collection of PrayerRequest objects.  It includes
	 ...
	 * 
	 */
	class PrayerRequestDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PrayerRequest datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PrayerRequest object, or it can be any QQNode extending from QQN::PrayerRequest().
		 * 
		 ...
		 * sorting using two fields from the PrayerRequest object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PrayerRequest
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PrayerRequest::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from PrayerRequest, given the clauses above
			$this->DataSource = PrayerRequest::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PrayerRequest-based QQNode.
		 * It will also verify that it is a proper PrayerRequest-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'prayer_request') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "prayer_request".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PrayerRequest()->Id;
				case 'Name': return QQN::PrayerRequest()->Name;
				case 'Email': return QQN::PrayerRequest()->Email;
				case 'Subject': return QQN::PrayerRequest()->Subject;
				case 'Content': return QQN::PrayerRequest()->Content;
				case 'IsConfidential': return QQN::PrayerRequest()->IsConfidential;
				case 'IsInappropriate': return QQN::PrayerRequest()->IsInappropriate;
				case 'IsAllowNotes': return QQN::PrayerRequest()->IsAllowNotes;
				case 'IsPrayerIndicator': return QQN::PrayerRequest()->IsPrayerIndicator;
				case 'PrayerCount': return QQN::PrayerRequest()->PrayerCount;
				case 'Date': return QQN::PrayerRequest()->Date;
				default: throw new QCallerException('Simple Property not found in PrayerRequestDataGrid content: ' . $mixContent);
