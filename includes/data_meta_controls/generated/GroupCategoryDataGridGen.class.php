<?php
	 ...
	 * listing a collection of GroupCategory objects.  It includes
	 ...
	 * 
	 */
	class GroupCategoryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this GroupCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the GroupCategory object, or it can be any QQNode extending from QQN::GroupCategory().
		 * 
		 ...
		 * sorting using two fields from the GroupCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from GroupCategory
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = GroupCategory::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from GroupCategory, given the clauses above
			$this->DataSource = GroupCategory::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a GroupCategory-based QQNode.
		 * It will also verify that it is a proper GroupCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'group_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "group_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'GroupId': return QQN::GroupCategory()->GroupId;
				case 'Group': return QQN::GroupCategory()->Group;
				case 'DateRefreshed': return QQN::GroupCategory()->DateRefreshed;
				case 'ProcessTimeMs': return QQN::GroupCategory()->ProcessTimeMs;
				default: throw new QCallerException('Simple Property not found in GroupCategoryDataGrid content: ' . $mixContent);
