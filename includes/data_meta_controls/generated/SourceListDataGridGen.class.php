<?php
	 ...
	 * listing a collection of SourceList objects.  It includes
	 ...
	 * 
	 */
	class SourceListDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SourceList datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SourceList object, or it can be any QQNode extending from QQN::SourceList().
		 * 
		 ...
		 * sorting using two fields from the SourceList object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SourceList
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SourceList::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SourceList-based QQNode.
		 * It will also verify that it is a proper SourceList-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'source_list') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "source_list".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SourceList()->Id;
				case 'Name': return QQN::SourceList()->Name;
				default: throw new QCallerException('Simple Property not found in SourceListDataGrid content: ' . $mixContent);
