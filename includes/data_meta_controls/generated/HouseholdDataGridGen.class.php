<?php
	 ...
	 * listing a collection of Household objects.  It includes
	 ...
	 * 
	 */
	class HouseholdDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Household datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Household object, or it can be any QQNode extending from QQN::Household().
		 * 
		 ...
		 * sorting using two fields from the Household object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Household
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Household::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Household, given the clauses above
			$this->DataSource = Household::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Household-based QQNode.
		 * It will also verify that it is a proper Household-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'household') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "household".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Household()->Id;
				...
				case 'CombinedStewardshipFlag': return QQN::Household()->CombinedStewardshipFlag;
