<?php
	 ...
	 * listing a collection of ClassTerm objects.  It includes
	 ...
	 * 
	 */
	class ClassTermDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ClassTerm datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ClassTerm object, or it can be any QQNode extending from QQN::ClassTerm().
		 * 
		 ...
		 * sorting using two fields from the ClassTerm object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ClassTerm
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ClassTerm::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ClassTerm, given the clauses above
			$this->DataSource = ClassTerm::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ClassTerm-based QQNode.
		 * It will also verify that it is a proper ClassTerm-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'class_term') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "class_term".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ClassTerm()->Id;
				case 'Name': return QQN::ClassTerm()->Name;
				case 'ActiveFlag': return QQN::ClassTerm()->ActiveFlag;
				default: throw new QCallerException('Simple Property not found in ClassTermDataGrid content: ' . $mixContent);
