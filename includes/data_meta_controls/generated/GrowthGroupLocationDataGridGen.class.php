<?php
	 ...
	 * listing a collection of GrowthGroupLocation objects.  It includes
	 ...
	 * 
	 */
	class GrowthGroupLocationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this GrowthGroupLocation datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the GrowthGroupLocation object, or it can be any QQNode extending from QQN::GrowthGroupLocation().
		 * 
		 ...
		 * sorting using two fields from the GrowthGroupLocation object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from GrowthGroupLocation
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = GrowthGroupLocation::CountAll();
			}
			...
			$this->DataSource = GrowthGroupLocation::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a GrowthGroupLocation-based QQNode.
		 * It will also verify that it is a proper GrowthGroupLocation-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'growth_group_location') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "growth_group_location".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::GrowthGroupLocation()->Id;
				case 'Location': return QQN::GrowthGroupLocation()->Location;
				case 'Longitude': return QQN::GrowthGroupLocation()->Longitude;
				case 'Latitude': return QQN::GrowthGroupLocation()->Latitude;
				case 'Zoom': return QQN::GrowthGroupLocation()->Zoom;
				default: throw new QCallerException('Simple Property not found in GrowthGroupLocationDataGrid content: ' . $mixContent);
