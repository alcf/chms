<?php
	 ...
	 * listing a collection of Email objects.  It includes
	 ...
	 * 
	 */
	class EmailDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Email datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Email object, or it can be any QQNode extending from QQN::Email().
		 * 
		 ...
		 * sorting using two fields from the Email object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Email
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Email::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Email, given the clauses above
			$this->DataSource = Email::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Email-based QQNode.
		 * It will also verify that it is a proper Email-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'email') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "email".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Email()->Id;
