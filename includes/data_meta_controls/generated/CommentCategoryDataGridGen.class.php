<?php
	 ...
	 * listing a collection of CommentCategory objects.  It includes
	 ...
	 * 
	 */
	class CommentCategoryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CommentCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CommentCategory object, or it can be any QQNode extending from QQN::CommentCategory().
		 * 
		 ...
		 * sorting using two fields from the CommentCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CommentCategory
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = CommentCategory::CountAll();
			}
			...
			$this->DataSource = CommentCategory::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CommentCategory-based QQNode.
		 * It will also verify that it is a proper CommentCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comment_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comment_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CommentCategory()->Id;
				case 'OrderNumber': return QQN::CommentCategory()->OrderNumber;
				case 'Name': return QQN::CommentCategory()->Name;
				default: throw new QCallerException('Simple Property not found in CommentCategoryDataGrid content: ' . $mixContent);
