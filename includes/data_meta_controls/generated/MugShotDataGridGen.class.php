<?php
	 ...
	 * listing a collection of MugShot objects.  It includes
	 ...
	 * 
	 */
	class MugShotDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MugShot datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MugShot object, or it can be any QQNode extending from QQN::MugShot().
		 * 
		 ...
		 * sorting using two fields from the MugShot object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MugShot
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = MugShot::CountAll();
			}
			...
			// Set the DataSource to be a Query result from MugShot, given the clauses above
			$this->DataSource = MugShot::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MugShot-based QQNode.
		 * It will also verify that it is a proper MugShot-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'mug_shot') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "mug_shot".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::MugShot()->Id;
				case 'PersonId': return QQN::MugShot()->PersonId;
				case 'Person': return QQN::MugShot()->Person;
				case 'DateUploaded': return QQN::MugShot()->DateUploaded;
				case 'PersonAsCurrent': return QQN::MugShot()->PersonAsCurrent;

				default: throw new QCallerException('Simple Property not found in MugShotDataGrid content: ' . $mixContent);
