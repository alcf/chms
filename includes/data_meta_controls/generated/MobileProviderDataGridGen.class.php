<?php
	 ...
	 * listing a collection of MobileProvider objects.  It includes
	 ...
	 * 
	 */
	class MobileProviderDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MobileProvider datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MobileProvider object, or it can be any QQNode extending from QQN::MobileProvider().
		 * 
		 ...
		 * sorting using two fields from the MobileProvider object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MobileProvider
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = MobileProvider::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MobileProvider-based QQNode.
		 * It will also verify that it is a proper MobileProvider-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'mobile_provider') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "mobile_provider".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::MobileProvider()->Id;
				case 'Name': return QQN::MobileProvider()->Name;
				case 'Domain': return QQN::MobileProvider()->Domain;
				default: throw new QCallerException('Simple Property not found in MobileProviderDataGrid content: ' . $mixContent);
