<?php
	 ...
	 * listing a collection of PaymentPeriod objects.  It includes
	 ...
	 * 
	 */
	class PaymentPeriodDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PaymentPeriod datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PaymentPeriod object, or it can be any QQNode extending from QQN::PaymentPeriod().
		 * 
		 ...
		 * sorting using two fields from the PaymentPeriod object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PaymentPeriod
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PaymentPeriod::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PaymentPeriod-based QQNode.
		 * It will also verify that it is a proper PaymentPeriod-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'payment_period') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "payment_period".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PaymentPeriod()->Id;
				case 'Name': return QQN::PaymentPeriod()->Name;
				default: throw new QCallerException('Simple Property not found in PaymentPeriodDataGrid content: ' . $mixContent);
