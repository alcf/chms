<?php
	 ...
	 * listing a collection of Praises objects.  It includes
	 ...
	 * 
	 */
	class PraisesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Praises datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Praises object, or it can be any QQNode extending from QQN::Praises().
		 * 
		 ...
		 * sorting using two fields from the Praises object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Praises
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Praises::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Praises, given the clauses above
			$this->DataSource = Praises::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Praises-based QQNode.
		 * It will also verify that it is a proper Praises-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'praises') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "praises".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Praises()->Id;
				case 'Name': return QQN::Praises()->Name;
				case 'Email': return QQN::Praises()->Email;
				case 'Subject': return QQN::Praises()->Subject;
				case 'Content': return QQN::Praises()->Content;
				case 'Date': return QQN::Praises()->Date;
				default: throw new QCallerException('Simple Property not found in PraisesDataGrid content: ' . $mixContent);
