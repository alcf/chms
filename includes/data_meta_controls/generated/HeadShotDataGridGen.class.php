<?php
	 ...
	 * listing a collection of HeadShot objects.  It includes
	 ...
	 * 
	 */
	class HeadShotDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this HeadShot datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the HeadShot object, or it can be any QQNode extending from QQN::HeadShot().
		 * 
		 ...
		 * sorting using two fields from the HeadShot object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from HeadShot
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = HeadShot::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a HeadShot-based QQNode.
		 * It will also verify that it is a proper HeadShot-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'head_shot') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "head_shot".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::HeadShot()->Id;
