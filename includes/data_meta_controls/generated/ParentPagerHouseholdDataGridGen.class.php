<?php
	 ...
	 * listing a collection of ParentPagerHousehold objects.  It includes
	 ...
	 * 
	 */
	class ParentPagerHouseholdDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ParentPagerHousehold datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ParentPagerHousehold object, or it can be any QQNode extending from QQN::ParentPagerHousehold().
		 * 
		 ...
		 * sorting using two fields from the ParentPagerHousehold object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ParentPagerHousehold
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ParentPagerHousehold::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ParentPagerHousehold-based QQNode.
		 * It will also verify that it is a proper ParentPagerHousehold-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'parent_pager_household') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "parent_pager_household".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ParentPagerHousehold()->Id;
				case 'ServerIdentifier': return QQN::ParentPagerHousehold()->ServerIdentifier;
				case 'HouseholdId': return QQN::ParentPagerHousehold()->HouseholdId;
				case 'Household': return QQN::ParentPagerHousehold()->Household;
				case 'HiddenFlag': return QQN::ParentPagerHousehold()->HiddenFlag;
				case 'ParentPagerSyncStatusTypeId': return QQN::ParentPagerHousehold()->ParentPagerSyncStatusTypeId;
				case 'Name': return QQN::ParentPagerHousehold()->Name;
				default: throw new QCallerException('Simple Property not found in ParentPagerHouseholdDataGrid content: ' . $mixContent);
