<?php
	 ...
	 * listing a collection of StewardshipPostAmount objects.  It includes
	 ...
	 * 
	 */
	class StewardshipPostAmountDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this StewardshipPostAmount datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the StewardshipPostAmount object, or it can be any QQNode extending from QQN::StewardshipPostAmount().
		 * 
		 ...
		 * sorting using two fields from the StewardshipPostAmount object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from StewardshipPostAmount
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = StewardshipPostAmount::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a StewardshipPostAmount-based QQNode.
		 * It will also verify that it is a proper StewardshipPostAmount-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'stewardship_post_amount') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "stewardship_post_amount".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::StewardshipPostAmount()->Id;
				case 'StewardshipPostId': return QQN::StewardshipPostAmount()->StewardshipPostId;
				case 'StewardshipPost': return QQN::StewardshipPostAmount()->StewardshipPost;
				case 'StewardshipFundId': return QQN::StewardshipPostAmount()->StewardshipFundId;
				case 'StewardshipFund': return QQN::StewardshipPostAmount()->StewardshipFund;
				case 'Amount': return QQN::StewardshipPostAmount()->Amount;
				default: throw new QCallerException('Simple Property not found in StewardshipPostAmountDataGrid content: ' . $mixContent);
