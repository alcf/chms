<?php
	 ...
	 * listing a collection of FormAnswer objects.  It includes
	 ...
	 * 
	 */
	class FormAnswerDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FormAnswer datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FormAnswer object, or it can be any QQNode extending from QQN::FormAnswer().
		 * 
		 ...
		 * sorting using two fields from the FormAnswer object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FormAnswer
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = FormAnswer::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FormAnswer-based QQNode.
		 * It will also verify that it is a proper FormAnswer-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'form_answer') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "form_answer".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::FormAnswer()->Id;
				case 'SignupEntryId': return QQN::FormAnswer()->SignupEntryId;
				...
				default: throw new QCallerException('Simple Property not found in FormAnswerDataGrid content: ' . $mixContent);
