<?php
	 ...
	 * listing a collection of ClassCourse objects.  It includes
	 ...
	 * 
	 */
	class ClassCourseDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ClassCourse datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ClassCourse object, or it can be any QQNode extending from QQN::ClassCourse().
		 * 
		 ...
		 * sorting using two fields from the ClassCourse object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ClassCourse
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ClassCourse::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ClassCourse, given the clauses above
			$this->DataSource = ClassCourse::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ClassCourse-based QQNode.
		 * It will also verify that it is a proper ClassCourse-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'class_course') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "class_course".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ClassCourse()->Id;
				case 'Code': return QQN::ClassCourse()->Code;
				case 'Name': return QQN::ClassCourse()->Name;
				default: throw new QCallerException('Simple Property not found in ClassCourseDataGrid content: ' . $mixContent);
