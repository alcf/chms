<?php
	 ...
	 * listing a collection of ClassMeeting objects.  It includes
	 ...
	 * 
	 */
	class ClassMeetingDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ClassMeeting datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ClassMeeting object, or it can be any QQNode extending from QQN::ClassMeeting().
		 * 
		 ...
		 * sorting using two fields from the ClassMeeting object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ClassMeeting
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intSignupFormId=<?=urlencode($_ITEM->SignupFormId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=str_replace("+","%20",urlencode($_ITEM->SignupFormId))?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->SignupFormId, false); ?>>' . QApplication::Translate($strLinkHtml) . '</a>';
			$colEditColumn = new QDataGridColumn(QApplication::Translate($strColumnTitle), $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ClassMeeting::QueryCount($objCondition, $objClauses);
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ClassMeeting-based QQNode.
		 * It will also verify that it is a proper ClassMeeting-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'class_meeting') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "class_meeting".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'SignupFormId': return QQN::ClassMeeting()->SignupFormId;
				case 'SignupForm': return QQN::ClassMeeting()->SignupForm;
				case 'ClassTermId': return QQN::ClassMeeting()->ClassTermId;
				case 'ClassTerm': return QQN::ClassMeeting()->ClassTerm;
				case 'ClassCourseId': return QQN::ClassMeeting()->ClassCourseId;
				case 'ClassCourse': return QQN::ClassMeeting()->ClassCourse;
				case 'ClassInstructorId': return QQN::ClassMeeting()->ClassInstructorId;
				case 'ClassInstructor': return QQN::ClassMeeting()->ClassInstructor;
				case 'DateStart': return QQN::ClassMeeting()->DateStart;
				case 'DateEnd': return QQN::ClassMeeting()->DateEnd;
				case 'Location': return QQN::ClassMeeting()->Location;
				case 'MeetingDay': return QQN::ClassMeeting()->MeetingDay;
				case 'MeetingStartTime': return QQN::ClassMeeting()->MeetingStartTime;
				case 'MeetingEndTime': return QQN::ClassMeeting()->MeetingEndTime;
				default: throw new QCallerException('Simple Property not found in ClassMeetingDataGrid content: ' . $mixContent);
