<?php
	 ...
	 * listing a collection of SignupProduct objects.  It includes
	 ...
	 * 
	 */
	class SignupProductDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this SignupProduct datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SignupProduct object, or it can be any QQNode extending from QQN::SignupProduct().
		 * 
		 ...
		 * sorting using two fields from the SignupProduct object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SignupProduct
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = SignupProduct::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from SignupProduct, given the clauses above
			$this->DataSource = SignupProduct::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SignupProduct-based QQNode.
		 * It will also verify that it is a proper SignupProduct-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'signup_product') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "signup_product".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SignupProduct()->Id;
				case 'SignupEntryId': return QQN::SignupProduct()->SignupEntryId;
