...
ALTER TABLE `phone` ADD FOREIGN KEY person_id_idxfk_8 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `email_message_route` ADD FOREIGN KEY email_message_id_idxfk (`email_message_id`) REFERENCES `email_message` (`id`);

CREATE INDEX `group_id_idx` ON `email_message_route`(`group_id`);
ALTER TABLE `email_message_route` ADD FOREIGN KEY group_id_idxfk (`group_id`) REFERENCES `group` (`id`);
...
ALTER TABLE `email_outgoing_queue` ADD FOREIGN KEY email_message_id_idxfk_1 (`email_message_id`) REFERENCES `email_message` (`id`);
