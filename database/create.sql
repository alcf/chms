...
`requires_distinct_flag` BOOLEAN,
PRIMARY KEY (`id`)
) ENGINE=InnoDB;

CREATE TABLE `query_condition_type`
(
`id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
`name` VARCHAR(40) NOT NULL UNIQUE,
PRIMARY KEY (`id`)
...
(
`id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
`search_query` INTEGER UNSIGNED NOT NULL,
`query_condition_type_id` INTEGER UNSIGNED NOT NULL,
`query_node` INTEGER UNSIGNED NOT NULL,
`value` VARCHAR(255),
PRIMARY KEY (`id`)
) ENGINE=InnoDB;
...
CREATE INDEX `search_query_idx` ON `query_condition`(`search_query`);
ALTER TABLE `query_condition` ADD FOREIGN KEY search_query_idxfk (`search_query`) REFERENCES `search_query` (`id`);

CREATE INDEX `query_condition_type_id_idx` ON `query_condition`(`query_condition_type_id`);
ALTER TABLE `query_condition` ADD FOREIGN KEY query_condition_type_id_idxfk (`query_condition_type_id`) REFERENCES `query_condition_type` (`id`);

CREATE INDEX `query_node_idx` ON `query_condition`(`query_node`);
ALTER TABLE `query_condition` ADD FOREIGN KEY query_node_idxfk (`query_node`) REFERENCES `query_node` (`id`);
...
ALTER TABLE `ministry_login_assn` ADD FOREIGN KEY ministry_id_idxfk (`ministry_id`) REFERENCES `ministry` (`id`);
...
ALTER TABLE `group_role` ADD FOREIGN KEY ministry_id_idxfk_2 (`ministry_id`) REFERENCES `ministry` (`id`);
...
ALTER TABLE `group_participation` ADD FOREIGN KEY person_id_idxfk (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `phone` ADD FOREIGN KEY person_id_idxfk_2 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `address` ADD FOREIGN KEY person_id_idxfk_3 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `relationship` ADD FOREIGN KEY person_id_idxfk_5 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `other_contact_info` ADD FOREIGN KEY person_id_idxfk_8 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `comment` ADD FOREIGN KEY person_id_idxfk_9 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `person_nameitem_assn` ADD FOREIGN KEY person_id_idxfk_10 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `email_message_route` ADD FOREIGN KEY person_id_idxfk_12 (`person_id`) REFERENCES `person` (`id`);
...
ALTER TABLE `marriage` ADD FOREIGN KEY person_id_idxfk_14 (`person_id`) REFERENCES `person` (`id`);
